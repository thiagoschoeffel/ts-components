  ...
  <template v-for="(entry, index) in items" :key="index">
    ...
    <DropdownMenuSub v-else-if="entry.type === 'submenu'">
      ...
      <DropdownMenuSubContent
        :side-offset="4"
        :align-offset="-4"
        class="z-[1] max-h-[var(--reka-dropdown-menu-content-available-height)] min-w-48 overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 text-sm shadow-xs outline-none">
        <DropdownMenuItems
          :items="entry.items"
